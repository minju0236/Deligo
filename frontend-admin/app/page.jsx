    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p className="auth-copy">배달 정산, 기사 계좌, 거래 내역, Redis 감사 로그를 관리하는 Deligo 운영 화면입니다.</p>
    ...
    <div className="admin-layout">
      <aside className="sidebar">
        ...
        <nav className="side-nav">
          ...
          <NavItem icon="계좌" label="정산 계좌 관리" />
          <NavItem icon="거래" label="정산 거래 모니터링" />
          <NavItem icon="로그" label="Redis 감사 로그" />
        </nav>
        <div className="side-footer">
          <span>Next Admin 3001</span>
          <small>Nginx Routed</small>
        </div>
      </aside>
      ...
      <div className="workspace">
        ...
        <main className="content">
          <section className="hero-card">
            <div>
              <span className="hero-label">Deligo Settlement Admin</span>
              <h2>배달 정산 데이터를 한 화면에서 관리합니다</h2>
              <p>Spring Boot API, MariaDB/JPA, Redis 세션/캐시/감사 로그를 기반으로 정산 운영 상태를 확인합니다.</p>
            </div>
          ...
          <section className="metric-grid">
            <div className="metric-card"><span>회원</span><strong>{data.userCount ?? users.length}</strong><p>등록된 운영 사용자</p></div>
            <div className="metric-card"><span>정산 계좌</span><strong>{data.accountCount ?? accounts.length}</strong><p>생성 및 관리 중인 계좌</p></div>
            <div className="metric-card"><span>거래</span><strong>{data.transactionCount ?? transactions.length}</strong><p>최근 정산 거래 내역</p></div>
            <div className="metric-card"><span>총 잔액</span><strong>{formatMoney(totalBalance)}</strong><p>정산 계좌 합산 금액</p></div>
          </section>
          ...
          <section className="operation-grid">
            ...
            <div className="panel">
              <div className="panel-head"><h2>정산 계좌 생성</h2><span>관리자 발급</span></div>
              <input placeholder="사용자 ID" value={account.userId} onChange={(e) => setAccount({ ...account, userId: Number(e.target.value) })} />
              <input placeholder="초기 금액" value={account.initialBalance} onChange={(e) => setAccount({ ...account, initialBalance: Number(e.target.value) })} />
            ...
            <div className="panel danger-panel">
              <div className="panel-head"><h2>상태/계좌 제어</h2><span>주의 작업</span></div>
          ...
          <section className="data-grid">
            ...
            <div className="table-panel wide">
              <div className="panel-head"><h2>정산 계좌 목록</h2><span>{accounts.length}건</span></div>
              <div className="table-wrap">
            ...
            <div className="table-panel">
              <div className="panel-head"><h2>최근 정산 거래</h2><span>{transactions.length}건</span></div>
              <div className="timeline">{transactions.slice(0, 8).map((t) => <div className="timeline-item" key={t.id}><div><strong>{t.type}</strong><p>{t.memo || "-"} · {t.fromAccountNumber || "출금 없음"} → {t.toAccountNumber || "-"}</p></div><b>{formatMoney(t.amount)}</b></div>)}</div>
            </div>
