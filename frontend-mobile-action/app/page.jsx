...
export default function MobileActionPage() {
  ...
  const [memo, setMemo] = useState("Deligo 정산 처리");
      ...
      <main className="mobile-shell action-shell">
        <section className="device-card login-card">
          ...
          <section className="signin-hero">
            ...
            <h2>로그인이 필요한 <br/> 서비스입니다</h2>
            <p>
              Deligo에 가입하여 사장님을 위한 배달 정산 서비스를 경험해보세요.
            </p>
          </section>
          ...
          <section className="form-card">
            ...
            <a className="subtle-link" href="/">정산 조회 화면으로 이동</a>
    ...
    <main className="mobile-shell action-shell">
      <section className="device-card">
        <div className="appbar">
          ...
          <button className="logout-btn" onClick={auth.logout}>로그아웃</button>
        </div>
        ...
        <section className="account-card google-blue-card">
          ...
          <h2>{account.data ? money(account.data.balance) : "정산 가능 금액 조회 중"}</h2>
          <p>{account.data?.accountNumber || "정산 계좌 정보를 불러오고 있습니다"}</p>
        </section>
        ...
        <section className="form-card transfer-card">
          ...
          <label>환불 대상 계좌</label>
          <input value={to} onChange={(e) => setTo(e.target.value)} placeholder="110-100-000002" />

          <label>처리 메모</label>
          <input value={memo} onChange={(e) => setMemo(e.target.value)} placeholder="처리 메모" />

          <div className="action-buttons">
            <button className="secondary-btn" disabled={isBusy} onClick={() => deposit.mutate({ amount: Number(amount), memo: memo || "주문 매출 적립" })}>매출 적립</button>
            <button className="secondary-btn" disabled={isBusy} onClick={() => withdraw.mutate({ amount: Number(amount), memo: memo || "정산 신청" })}>정산 신청</button>
            <button className="primary-btn span2" disabled={isBusy} onClick={() => transfer.mutate({ toAccountNumber: to, amount: Number(amount), memo: memo || "환불 처리" })}>환불 처리</button>
            <button className="dark-btn span2" disabled={isBusy} onClick={() => multi.mutate({ memo: "일괄 정산", targets: [{ toAccountNumber: "110-100-000002", amount: 1000 }, { toAccountNumber: "110-100-000003", amount: 2000 }] })}>여러 사장님에게 일괄 정산</button>
          </div>
        ...
        {result && (
          <section className="result-card">
            ...
            <p>현재 정산 가능 금액 {money(result.account?.balance)}</p>
          </section>
        )}
        ...
        <section className="list-card">
          <div className="section-title">
            ...
            <span className="server-chip">Redis 정산 세션</span>
          </div>
            ...
            {(tx.data || []).slice(0, 6).map((item) => (
              <div className="tx-item" key={item.id}>
                <div>
                  ...
                  <strong>{item.memo || "정산 처리"}</strong>
                  <p>{item.createdAt}</p>
                </div>
                <b>{money(item.amount)}</b>
              </div>
            ))}
            {!(tx.data || []).length && <p className="empty">정산 이력이 없습니다.</p>}
          </div>
        </section>

        <nav className="bottom-nav">
          <a href="/">홈</a>
          <span className="active">정산</span>
          <a href="/">이력</a>
